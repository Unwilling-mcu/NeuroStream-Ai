...
export default function VideoCard({ video, isActive, onClick, onQueue, onRemove, progress }) {
  ...
  const [thumbLoading, setThumbLoading] = useState(!video.thumbnail && !!video.file_path);
  ...
  // Lazy thumbnail fetch from backend
  useEffect(() => {
    if (thumbnail || thumbFetched.current || !video.file_path || video.isNetwork) {
    ...
    fetch(`http://localhost:5000/api/thumbnail-generate?file_path=${encodeURIComponent(video.file_path)}`)
      ...
      .then(d => { if (d?.url) setThumbnail(d.url); })
      .catch(() => {})
    ...
    <>
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", top: "6px", left: "6px", display: "flex", gap: "4px", flexWrap: "wrap" }}>
            {isActive && (
              <span style={{ background: "var(--red)", color: "#fff", fontSize: "8px", fontWeight: 800, padding: "2px 7px", borderRadius: "4px", letterSpacing: "0.06em" }}>
                NOW PLAYING
              </span>
            )}
            {video.isNetwork && (
              <span style={{ background: "#4a9eff", color: "#fff", fontSize: "8px", fontWeight: 800, padding: "2px 7px", borderRadius: "4px" }}>
                NETWORK
              </span>
            )}
            {qb && (
              <span style={{ background: "rgba(0,0,0,0.75)", color: qb.color, fontSize: "9px", fontWeight: 700, padding: "2px 6px", borderRadius: "4px", border: `1px solid ${qb.color}55` }}>
                {qb.label}
              </span>
            )}
          </div>
        ...
        <div style={{ padding: "9px 11px 10px" }}>
          <p style={{
            margin: "0 0 3px", fontSize: "12.5px", fontWeight: 600,
          ...
          <div style={{ display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
            {video.resolution && video.resolution !== "Unknown" && (
              <span style={{ fontSize: "10px", color: "var(--text3)" }}>{video.resolution}</span>
            )}
            {video.size > 0 && (
              <span style={{ fontSize: "10px", color: "var(--text3)" }}>{fmtSize(video.size)}</span>
            )}
            {progressPct > 2 && progressPct < 98 && (
              <span style={{ fontSize: "10px", color: "var(--red)", marginLeft: "auto" }}>
        ...
        {contextMenu && (
          ...
          >
            {[
              { icon: "▶",  label: "Play now",         action: () => { onClick(); setContextMenu(null); } },
              ...
              <div
                ...
                style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 12px", borderRadius: "7px", fontSize: "12.5px", color: "#ccc", cursor: "pointer", transition: "background 0.12s" }}
            ...
            {onRemove && (
              <>
                ...
                <div
                  onClick={e => { e.stopPropagation(); setContextMenu(null); setConfirmDelete(true); }}
                  style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 12px", borderRadius: "7px", fontSize: "12.5px", color: "#ff6b6b", cursor: "pointer", transition: "background 0.12s" }}
      ...
      {confirmDelete && (
        <div
          ...
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(8px)" }}
        >
          <div
            onClick={e => e.stopPropagation()}
          ...
          >
            ...
            <p style={{ margin: "0 0 6px", fontSize: "13px", color: "var(--text2)" }}>
              <strong style={{ color: "#fff" }}>{video.title}</strong>
            </p>
            <p style={{ margin: "0 0 24px", fontSize: "12px", color: "var(--text3)" }}>
              The actual file on your disk is <strong>not deleted</strong>.
            </p>
            <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
              <button
                onClick={() => setConfirmDelete(false)}
                style={{ flex: 1, padding: "10px", background: "rgba(255,255,255,0.06)", border: "1px solid var(--border)", color: "#fff", borderRadius: "9px", cursor: "pointer", fontSize: "13px", fontWeight: 600 }}
              >Cancel</button>
              <button
                onClick={e => { e.stopPropagation(); onRemove?.(video); setConfirmDelete(false); }}
                style={{ flex: 1, padding: "10px", background: "rgba(229,9,20,0.15)", border: "1.5px solid rgba(229,9,20,0.5)", color: "#e50914", borderRadius: "9px", cursor: "pointer", fontSize: "13px", fontWeight: 700 }}
